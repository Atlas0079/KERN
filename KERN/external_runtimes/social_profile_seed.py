...
SCIENCE_VIDEO_PLATFORM_ARCHETYPES: WeightedOptions = {
	"private_social": 0.18,
	"short_video_mass": 0.10,
	"public_discussion": 0.27,
	"lifestyle_discovery": 0.06,
	"interest_community": 0.39,
}
...
SCIENCE_VIDEO_AGE_BANDS: WeightedOptions = {
	"25-34": 0.40,
	"35-44": 0.30,
	"45-54": 0.18,
	"55+": 0.12,
...
SCIENCE_VIDEO_TOPICS: WeightedOptions = {
	"science_environment": 0.24,
	"science_health": 0.20,
	"science_technology": 0.23,
	"science_society": 0.18,
	"science_history": 0.15,
}
...
class SocialProfileSampler:
	def __init__(self, seed: int | str | None = None, audience: str = "general"):
		if audience not in {"general", "science_video"}:
			raise ValueError(f"unsupported social profile audience: {audience}")
		self.rng = random.Random(seed)
		self.audience = audience
		self.platform_archetypes = SCIENCE_VIDEO_PLATFORM_ARCHETYPES if audience == "science_video" else PLATFORM_ARCHETYPES
		self.age_bands = SCIENCE_VIDEO_AGE_BANDS if audience == "science_video" else AGE_BANDS
	...
	def sample_profile(self, index: int = 0, include_debug: bool = False) -> dict[str, Any]:
		...
		sample["platform_archetype"] = _weighted_choice(self.rng, self.platform_archetypes)
		weights_debug["platform_archetype"] = {k: round(v, 4) for k, v in self.platform_archetypes.items()}
		trace.append({"field": "platform_archetype", "reason": f"{self.audience} audience platform mix prior", "weights": dict(self.platform_archetypes)})

		age_weights, age_trace = _apply_rules(self.age_bands, "age_band", sample, PLATFORM_INFLUENCE_RULES)
		...
		if self.audience == "science_video":
			sample["education"] = _weighted_choice(self.rng, {"bachelor": 0.68, "graduate": 0.32})
			sample["media_style"] = _weighted_choice(self.rng, {"longform_reader": 0.38, "podcast_audio": 0.28, "news_commentary": 0.22, "short_video_scroller": 0.07, "visual_lifestyle": 0.05})
		...
		if self.audience == "science_video":
			sample["science_video_topics"] = _label_list(_sample_many(self.rng, SCIENCE_VIDEO_TOPICS, 2))
		...
		if self.audience == "science_video":
			profile["audience_preset"] = "science_video"
		return profile
	...
	def _specific_occupation(self, sample: dict[str, Any]) -> tuple[str, dict[str, float], list[dict[str, Any]]]:
		...
		weights = self._filter_specific_occupation_weights(weights, sample)
		return _weighted_choice(self.rng, weights), {k: round(v, 4) for k, v in weights.items()}, trace

	def _filter_specific_occupation_weights(self, weights: WeightedOptions, sample: dict[str, Any]) -> WeightedOptions:
		if str(sample.get("occupation_domain", "")) != "student":
			return weights
		allowed = {
		...
		}.get(str(sample.get("education", "")), set())
		specific_education = str(sample.get("_specific_education", sample.get("specific_education", "")))
		if specific_education in {"普通本科毕业", "师范类本科毕业", "计算机相关本科", "财经类本科", "艺术设计本科", "民办本科毕业", "硕士毕业", "在职研究生毕业", "专业硕士毕业", "博士肄业后工作", "海外硕士毕业"}:
			allowed -= {"本科生", "大专生", "职业培训学员"}
		if specific_education == "大专毕业":
			allowed -= {"大专生"}
		if not allowed:
			return weights
		return {key: value for key, value in weights.items() if key in allowed}
	...
	def _sample_specifics(self, sample: dict[str, Any]) -> tuple[dict[str, Any], dict[str, dict[str, float]], list[dict[str, Any]]]:
		...
		education_weights, education_trace = self._specific_value_weights(sample, "education", str(sample.get("education", "")), "specific_education")
		if age < 20:
			education_weights = {key: value for key, value in education_weights.items() if key not in {"大专毕业", "普通本科毕业", "师范类本科毕业", "计算机相关本科", "财经类本科", "艺术设计本科", "民办本科毕业", "硕士毕业", "在职研究生毕业", "专业硕士毕业", "博士肄业后工作", "海外硕士毕业"}}
		elif age < 22:
			education_weights = {key: value for key, value in education_weights.items() if key not in {"硕士毕业", "在职研究生毕业", "专业硕士毕业", "博士肄业后工作", "海外硕士毕业"}}
		education = _weighted_choice(self.rng, education_weights)
		occupation_context = dict(sample)
		occupation_context["age"] = age
		occupation_context["_specific_education"] = education
		occupation, occupation_weights, occupation_trace = self._specific_occupation(occupation_context)
		...
		return {
			...
			"science_video_topics": [
				{"id": str(item.get("id", "")), "label": str(item.get("label", "")), "specific": _specific_interest(self.rng, str(item.get("id", "")))}
				for item in list(sample.get("science_video_topics", []) or [])
...
def generate_social_profiles(count: int = 100, seed: int | str | None = None, include_debug: bool = False, audience: str = "general") -> list[dict[str, Any]]:
	sampler = SocialProfileSampler(seed=seed, audience=audience)
