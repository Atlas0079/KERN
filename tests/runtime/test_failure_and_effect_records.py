...
from KERN.agent_workflow.context_builder import LLMDecisionContext
from KERN.agent_workflow.contracts import TurnFrame
...
class FailureAndEffectRecordTests(unittest.TestCase):
	...
	def test_llm_failure_keeps_raw_failure_evidence(self) -> None:
		...
		frame = TurnFrame(actor_id="agent", reason="tick", mode_context={})
		decision_context = LLMDecisionContext(perception={"tick": 1, "location": {"id": "room"}}, action_catalog={})
		with self.assertRaises(KernFailure) as caught:
			provider._decide_frame(frame, decision_context)
