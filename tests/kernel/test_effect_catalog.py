...
def _record_ping(ws, data, context, events):
	return f"recorded {data.get('effect')} with {len(events)} events"
...
class EffectCatalogTests(unittest.TestCase):
	...
	def test_catalog_resolves_explicit_recorder(self) -> None:
		catalog = EffectCatalog()
		catalog.register(
			EffectSpec(
				effect_id="test:Ping",
				binder=_bind_passthrough,
				handler=_execute_ping,
				recorder=_record_ping,
			)
		)

		self.assertIs(catalog.resolve_recorder("test:Ping"), _record_ping)
